...
When a major key plays its fourth degree both major and minor (`G` and `Gm` in D major — the borrowed minor plagal), `minorFourthScales()` adds the two scales that fit the loan: the major pentatonic a major third under the iv (E♭ major pentatonic, whose five notes all sit in G natural minor) and the minor pentatonic of the iv itself. Both fall outside the key, so each is spelled after its own root rather than after the signature of the song.
